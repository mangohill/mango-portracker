...
const WORKER_CODE = [
  ...
  "}",
  "",
  "async function runFullBackfill(env, since) {",
  ...
  "",
  "  const [asxResults, fx] = await Promise.all([",
  "    Promise.all(asxSyms.map(async sym => [sym, await fetchYahooHistory(sym)])),",
  "    fetchFXHistory(since),",
  "  ]);",
  "  const fxDates = Object.keys(fx).sort();",
  "",
  "  const cryptoResults = [];",
  "  for (const id of cryptoIds) {",
  "    const sym = CG_ID_TO_SYM[id];",
  "    if (!sym) { cryptoResults.push([id, {}, false]); continue; } // unmapped id, skip",
  "    const pair = toBinancePair(sym);",
  "    if (!pair) { cryptoResults.push([id, {}, true]); continue; } // stablecoin",
  "    cryptoResults.push([id, await fetchBinanceHistory(pair, sinceMs), false]);",
  "  }",
  "",
  "  const blob = {};",
  "  // ASX — Yahoo already returns AUD, store keyed exactly like the cron",
  "  // does (ticker incl. .AX), so priceHistory's client-side normalisation",
  "  // needs no changes.",
  "  for (const [sym, series] of asxResults) {",
  "    for (const [dateKey, price] of Object.entries(series)) {",
  "      if (dateKey < since) continue;",
  "      (blob[dateKey] ||= {})[sym] = price;",
  "    }",
  "  }",
  "  // Crypto — keyed by CoinGecko id (matches cron format), converted",
  "  // USDT → AUD via the FX table. Stablecoins approximate to the FX rate",
  "  // itself (~1 USD).",
  "  for (const [id, series, isStable] of cryptoResults) {",
  "    if (isStable) {",
  "      for (const dateKey of fxDates) {",
  "        if (dateKey < since) continue;",
  "        (blob[dateKey] ||= {})[id] = fx[dateKey];",
  "      }",
  "      continue;",
  "    }",
  "    for (const [dateKey, priceUsdt] of Object.entries(series)) {",
  "      if (dateKey < since) continue;",
  "      const rate = fxRateFor(dateKey, fx, fxDates);",
  "      if (!rate) continue;",
  "      (blob[dateKey] ||= {})[id] = priceUsdt * rate;",
  "    }",
  "  }",
  "",
  ...
  "    asxSymbols: asxSyms,",
  "    cryptoIds,",
  "    daysStored: Object.keys(blob).length,",
  "  };",
  "}",
  "",
  "export default {",
  ...
  "    }",
  "",
  "    // ── ONE-OFF FULL BACKFILL (?backfillFull=1&since=2017-08-07) ───",
  "    // Fetches complete history for currently-held symbols (ASX via Yahoo,",
  "    // crypto via Binance + FX conversion to AUD) and stores it as a single",
  "    // KV blob. Run manually once (or again later to extend/refresh) — NOT",
  "    // called by the cron. Takes a few seconds; safe to re-run.",
  "    if (url.searchParams.get(\"backfillFull\") === \"1\") {",
  "      const since = url.searchParams.get(\"since\") || \"2017-08-07\";",
  "      try {",
  "        const result = await runFullBackfill(env, since);",
