...
const WORKER_CODE = [
  ...
  "    const series = await fetchYahooHistory(ticker);",
  "    let count = 0;",
  "    for (const [d, priceUsd] of Object.entries(series)) {",
  "      if (d < since) continue;",
  "      const rate = fxRateFor(d, fx, fxDates);",
  "      if (!rate) continue;",
  "      (blobDelta[d] ||= {})[item.key] = priceUsd * rate;",
  "      count++;",
  "    }",
  "    report.push({ symbol: item.key, type: 'crypto', ticker, days: count, rawDays: Object.keys(series).length });",
  "  }",
  "",
